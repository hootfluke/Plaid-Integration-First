    ...
    def validate(self, data):
        ...
        try:
            ...
            if not email or not public_token:
                raise ValidationError('Invalid Credentials or Token Exchange')
        except Exception as e:
            raise ValidationError(str(e))
...
class GetTransactionSerializer(serializers.Serializer):
    ...
    transacions = serializers.JSONField(required=False, read_only=True)
    ...
    def validate(self, data):
        ...
        except plaid.errrors.PlaidError as e:
            raise ValidationError(str(e))
        output['email'] = email
